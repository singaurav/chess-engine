...
namespace {
  ...
  void Evaluation<T>::initialize(bool force_eval) {
  ...
  template<Tracing T>
  Value Evaluation<T>::value(bool force_eval) {
    ...
    if (me->specialized_eval_exists() && !force_eval)
    ...
    if (abs(v) > LazyThreshold && !force_eval)
...
std::string Eval::trace(const Position& pos) {
  ...
  Value v = Evaluation<TRACE>(pos).value(false);
